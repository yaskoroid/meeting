    ...
    c\Typable
{
    ...
    protected function rawDelete(
        ...
        // Skoroid begin
        //old $name       = $collection->getName();
        $name      = $this->getStyle()->realProperty($collection->getName());
        // Skoroid end
    ...
    protected function createStatement(
        ...
        try {
            ...
            throw new \Exception('Error in DB query: '.$queryString.' with params: '.json_encode($queryParams, JSON_UNESCAPED_UNICODE), 0, $t);
        } catch (\Exception $e) {
            throw new \Exception('Error in DB query: '.$queryString.' with params: '.json_encode($queryParams, JSON_UNESCAPED_UNICODE), 0, $e);
        }
        return $statement;
    }
    ...
    protected function extractColumns($entity, Collection $collection)
    {
        ...
        // Skoroid begin
        //old $cols = get_object_vars($entity);
        $colsMy = get_object_vars($entity);

        $cols = array();
        foreach ($colsMy as $key=>$columnValue) {
            $cols[$this->getStyle()->realProperty($key)] = $columnValue;
        }
        // Skoroid end
    ...
    protected function inferSet(&$entity, $prop, $value)
    {
        ...
        try {
            // Skoroid begin
            $newProp = $this->getStyle()->styledProperty($prop);

            //$mirror = new \ReflectionProperty($entity, $prop);
            $mirror = new \ReflectionProperty($entity, $newProp);
            // Skoroid end
