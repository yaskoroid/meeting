...
class Email extends Base {
    ...
    public function create($userEmailTo, $type, array $paramsForTemplate = null, $hash = null) {
        ...
        $email = new Ent\Email();
        $email->emailFromUserId = $currentUser->email;
        $email->emailToUserId   = $userEmailTo;
        $email->type            = $type;
    ...
    /**
     * @param Ent\Email $email
     * @return bool
     * @throws \InvalidArgumentException
     */
    public function send($email, $name = null, $surname = null) {

        if (empty($email->emailFromUserId) || empty($email->emailToUserId))
            throw new \InvalidArgumentException('Email user send to or user send from is empty');

        $userFrom = $this->_userService->getUserByEmail($email->emailFromUserId);
        $from = '';
        if ($userFrom !== null)
            $from = "{$userFrom->name} {$userFrom->surname}";

        $userTo = $this->_userService->getUserByEmail($email->emailToUserId);
        $to = "{$name} {$surname}";
        if ($userTo !== null)
        ...
        $mailer->setFrom($email->emailFromUserId, $from);
        $mailer->addAddress($email->emailToUserId, $to);
